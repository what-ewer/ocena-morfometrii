...
def read_file_with_no_dims(file):
    itkimage = sitk.ReadImage(file[:-4] + '.mhd')
    ct_scan = sitk.GetArrayFromImage(itkimage)
    spacing = np.array(list(reversed(itkimage.GetSpacing())))
    
    print("specimen shape: " + str(ct_scan.shape))
    print("specimen spacing: " + str(itkimage.GetSpacing()))
    return ct_scan
...
class ColorMapVisualizer:
    ...
    def get_rainbow_color_function(self, gradient):
        ...
        if gradient:
            ...
            volume_color.AddRGBPoint(self._data_scalar_range[1], *(1, 0, 1))
